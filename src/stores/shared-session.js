...
export const useSharedSessionStore = defineStore("sharedSession", {
  state: () => ({
    sharedSession: null,
  ...
  actions: {
    async createSharedSession(exercise, partner) {
      ...
      return withPersistence({
        e2e: () => null,
        remote: async () => {
          const exerciseName = trimName(exercise.name ?? "");
          const exerciseNameLower = trimAndLowercase(exercise.name ?? "");
          ...
          const session = {
            ...
            // Tracks who started the round rotation to decide rest timing.
          ...
          try {
            ...
            if (!partnerSnap.empty) {
              ...
              const match = allSnap.docs.find((docSnap) => {
                ...
                return trimAndLowercase(data.name ?? "") === exerciseNameLower;
              });
              if (match) {
    ...
    async completeSharedSet(session, exercise) {
      ...
      await runTransaction(db, async (transaction) => {
        ...
        if (!shouldFinishSession) {
          const newRoundStartUid = shouldUpdateRoundStart ? nextBro.uid : roundStartUid;
          transaction.update(sessionRef, {
